...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      "Submit your MCP endpoint definitions. Call this when you've explored enough to define the planned tools. Each endpoint = name + description + ordered steps + metadata (dependencies, auth, etc).",
    input_schema: {
      type: "object" as const,
      properties: {
        endpoints: {
          ...
          items: {
            ...
            properties: {
              ...
              metadata: {
                type: "object",
                description: "Dependency and execution info for this endpoint",
                properties: {
                  dependsOn: {
                    type: "array",
                    items: { type: "string" },
                    description: "Names of endpoints that must run before this one in a flow (e.g. add_to_cart depends on search_products or get_product_details)",
                  },
                  provides: {
                    type: "array",
                    items: { type: "string" },
                    description: "State tokens this endpoint provides (e.g. 'cart_state', 'logged_in', 'search_results')",
                  },
                  requiresAuth: {
                    type: "boolean",
                    description: "Whether this endpoint needs the user to be signed in",
                  },
                },
              },
            },
  ...
  return obj.endpoints
    ...
    .map((e: unknown) => {
      ...
      const metadata: EndpointMetadata | undefined = rawMeta
        ? {
            dependsOn: Array.isArray(rawMeta.dependsOn)
              ? (rawMeta.dependsOn as string[])
              : undefined,
            provides: Array.isArray(rawMeta.provides)
              ? (rawMeta.provides as string[])
              : undefined,
            requiresAuth: typeof rawMeta.requiresAuth === "boolean"
              ? rawMeta.requiresAuth
              : undefined,
...
- You have plenty of turns — explore thoroughly but don't waste calls on pages that don't help you build the planned tools.
...
This lets endpoints be chained in flows. For example:
  search_products (provides: ["search_results"]) → get_product_details (dependsOn: ["search_products"]) → add_to_cart (provides: ["cart_state"]) → checkout (dependsOn: ["cart_state"], requiresAuth: true)
...
Submit your endpoints when you have covered the most important tools from the plan.`;
  ...
  while (turnCount < maxTurns) {
    ...
    // If agent hasn't submitted after many browser calls, force it
